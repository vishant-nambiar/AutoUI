...
def build_argparser():
    ...
    parser.add_argument("-flag", "--visualization_flag", required=False, nargs='+',
                        default=[],
                        help="Example: --flag fdm flm hp gem (Seperate each flag by space)"
                             "for see the visualization of different model outputs of each frame,"
                             "fdm for Face Detection Model, flm for Facial Landmark Detection Model"
                             "hpm for Head Pose Estimation Model, gem for Gaze Estimation Model.")
    return parser
    ...
    for flag, frame in input_feeder.next_batch():
        ...
        if len(oneneneflags) != 0:
            ...
            if 'gem' in oneneneflags:
                ...
                if 'fdm' in oneneneflags or 'flm' in oneneneflags:
